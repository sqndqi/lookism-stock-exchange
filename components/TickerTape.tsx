...
export function TickerTape() {
  return (
    <section className="relative z-20 border-y border-white/10 bg-black/70 py-3 backdrop-blur-xl">
      <div className="flex animate-ticker gap-8 whitespace-nowrap">
        {tickerTape.map((item, index) => (
          <div key={`${item.symbol}-${index}`} className="flex items-center gap-3 font-mono text-sm uppercase tracking-[0.14em]">
            ...
            <span className={item.change >= 0 ? "text-profit" : "text-danger"}>{signedPercent(item.change)}</span>
          </div>
        ))}
      </div>
    </section>
