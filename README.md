...
- Mission/progression actions grant cosmetic XP only. They do not grant fake credits.
- Account state is local browser state, not a backend account

Account types live in `lib/account.ts`.
The account schema is versioned. `readAccount()` normalizes older localStorage desks and writes a backup copy to `ptj-account-backup` before migration. Corrupt active account JSON is quarantined under `ptj-account-corrupt` instead of crashing the app. Export/import tools validate and normalize JSON before overwriting the active local desk.
